...
def generate_welcome_message(articles: list[dict]) -> str:
    ...
    genai.configure(api_key=GENAI_API_KEY) # Corrected: 'api_key'
    model = genai.GenerativeModel("gemini-2.5-flash") # Using gemini-2.5-flash as per your last update
...
def summarise_rss(articles: list[dict], bullets: int = 5) -> list[dict]:
    ...
    model = genai.GenerativeModel("gemini-2.5-pro") # Using gemini-2.5-pro as per your last update for summarization
    ...
    for article in articles[:bullets]:
        ...
        try:
            ...
            if not rundown_text and not details_html:
                ...
                # This HTML explicitly adds "The Radar:"
...
Here's the consolidated and corrected code for your `bot/agent.py` file. I've incorporated all the prompt refinements, model choices (`gemini-2.5-flash` for welcome, `gemini-2.5-pro` for summaries), and the logic for the "Radar" and bullet points.

**One critical error was identified in the HTML: missing closing single quote on a `padding` attribute value.** I've fixed `padding: 0 25px;'` to `padding: 0 25px;` in the HTML digest.
